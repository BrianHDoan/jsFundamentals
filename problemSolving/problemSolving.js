...
/* 
1) Create a function
2) add for statement in function
3) 
*/

let veggAmounts = [{name:'Sweet Potatoes',amount:4},{name:'Heirloom Tomatoes',amount:15},{name:'Kolrabi',amount:2},{name:'HoneyCombs',amount:1}]

let veggPrices = [{name:'Sweet Potatoes',price:2},{name:'Heirloom Tomatoes',price:1.50},{name:'Kolrabi',price:4},{name:'HoneyCombs',price:10}]

function price() {
    for (let i = 0; i < veggAmounts.length; i++) {
        if (veggPrices[i].name == veggAmounts[i].name) {
...
let ourNames = ['Brian', 'Megan'];

function societyName (nameArray) {
  nameArray.sort();
